...
function lottery_ticket_generation(GRB_ENV, ticket_dir, RWAFileName, LPFileName, TicketFileName, IPTopo, IPScenarios, OpticalTopo, OpticalScenarios, scenario_generation_only, optical_rerouting_K, option_num, option_gap, scenario_id, topology_index, verbose, ticket_or_not, progress0)
    ...
    for q in 1:length(IPScenarios["code"])
        ...
        println("simulator line 67,IPScenarios[code][q]", replace(string(IPScenarios["code"][q]),","=>""))
        println("sum(IPScenarios[code][q]) < length(IPScenarios[code][q])", sum(IPScenarios["code"][q]) , ",", length(IPScenarios["code"][q]))
        ...
        if sum(IPScenarios["code"][q]) < length(IPScenarios["code"][q])
            ...
            failed_IPedge, failed_IP_initialindex, failed_IP_initialbw, failed_IP_transponders_num = ReadFailureScenario(IPScenarios["code"][q], IPTopo["links"], IPTopo["capacity"], IPTopo["link_spectrum_center_flexgrid"])
            ...
            #flexgrid restoration here
            # WaveRerouting_flexgrid(OpticalTopo, IPTopo, failed_IPedge, failed_IP_initialindex, failed_fibers_index, optical_rerouting_K)
            ...
            rwa_runtime = @timed restored_bw_rwa, obj, IPBranch_bw = RestoreILP(GRB_ENV, OpticalTopo["links"], OpticalTopo["capacityCode"], failed_IPedge, rehoused_IProutingEdge, failedIPbranckindex, failedIPbrachGroup, failed_IP_initialbw, optical_rerouting_K,  OpticalTopo["fiber_length"], failed_IP_transponders_num)
            ...
            for i in 1:length(failed_IP_initialindex)
                rwa_full_capacity[failed_IP_initialindex[i]] = Int(round(restored_bw_rwa[i])) 
            end
            rwa_current_scenario_restored_bw = rwa_full_capacity  # network capacity vector of all links after rwa restoration
            println("wavelength continuous ILP full capacity: ", rwa_current_scenario_restored_bw)
            ...
            for i in 1:length(failed_IP_initialindex)
                rwa_lp_full_capacity[failed_IP_initialindex[i]] = Int(round(lp_restored_bw[i])) 
            end
            ...
            if ticket_or_not == 2  
                ...
                # 选择ILP的rwa的结果作为ticket，因此将rwa直接赋值给 rr_scenario_restored_bw (rr_scenario_restored_bw本来是由lp的结果经过四舍五入后得到的option_num张ticket)
                println(" here in simulator line 117, use rwa_full_capacity as tickets")
                rr_current_scenario_restored_bw_0 = deepcopy(rwa_full_capacity)
    ...
    if scenario_generation_only == 0
        println(" here in simulator line 191")
...
function get_failure_scenarios(dir, topology, topology_index, verbose, scenario_cutoff, scenario_id, weibull_or_k, failure_free, expanded_spectrum, state_id)
    ...
    # end
    weibull_failure = false
    
    ## Get the network topology
    IPTopo, OpticalTopo = ReadCrossLayerTopo(dir, topology, topology_index, verbose, expanded_spectrum, state_id, weibull_failure=weibull_failure, IPfromFile=true, tofile=false)
    ...
    if failure_free
        ...
        scenario_cutoff_format=format(scenario_cutoff)
        if verbose println("Computing fiber cut scenarios...cutoff=$(scenario_cutoff_format)") end
        failureFileName =  "./data/topology/$(topology)/IP_topo_$(topology_index)/$(scenario_cutoff_format)_ip_scenarios_$(scenario_id).jld"
        ...
        println("failureFileName exit")
        JLD.save(failureFileName, "IPScenarios", IPScenarios, "OpticalScenarios", OpticalScenarios)
        open("./data/topology/$(topology)/IP_topo_$(topology_index)/$(scenario_cutoff_format)_ip_scenarios_$(scenario_id).txt", "w+") do io
            for i in 1:size(IPScenarios["code"],1)
                writedlm(io, (IPScenarios["prob"][i], IPScenarios["code"][i]))
            end
        end
...
function simulator(GRB_ENV, dir, AllTopologies, AllTopoIndex, AllTunnelNum, AllAlgorithms, All_demand_upscale, All_demand_downscale, AllTraffic, scenario_cutoff, beta, scales, optical_rerouting_K, partial_load_full_tickets, option_num, large_option_num, option_gap, verbose, parallel_dir, singleplot, scenario_id, tunnel_rounting, failure_simulation, failure_free, expanded_spectrum)
    ...
    for t in 1:length(AllTopologies)
        for ii in 1:length(AllTopoIndex)
            ...
            weibull_or_k = 1  # does not affect because we do not generate tickets here
            ...
            IPTopo, OpticalTopo, IPScenarios, OpticalScenarios = get_failure_scenarios(dir, topology, topology_index, verbose, scenario_cutoff, scenario_id, weibull_or_k, failure_free, expanded_spectrum, 4)
            IPTopo_0, OpticalTopo_0, IPScenarios_0, OpticalScenarios_0 = IPTopo, OpticalTopo, IPScenarios, OpticalScenarios
            ...
            # println("IPTopo[capacity_flexgrid]:", IPTopo["capacity_flexgrid"])
            ...
            # state_prob = readdlm("data/topology/B4/state_prob.txt", header=false)
            # println("state_prob: ",state_prob)
            # state_num = size(state_prob,1)
            # failureFileName =  "./data/topology/$(topology)/Scenarios_$(state_num)_$(scenario_cutoff_format)_$(state_prob[1]).jld"
            # if isfile(failureFileName) == false
            #     IPTopo_list, OpticalTopo_list, IPScenarios_list, OpticalScenarios_list = [], [], [], []
            #     for state_id in 1:state_num
            #         IPTopo, OpticalTopo, IPScenarios, OpticalScenarios = get_failure_scenarios(dir, topology, topology_index, verbose, scenario_cutoff, scenario_id, weibull_or_k, failure_free, expanded_spectrum, state_id+5)
            #         # println("IPScenarios_prob",IPScenarios["prob"])
            #         push!(IPTopo_list,IPTopo)
            #         push!(OpticalTopo_list,OpticalTopo)
            #         push!(IPScenarios_list,IPScenarios)
            #         push!(OpticalScenarios_list,OpticalScenarios)
            #     end
            #     JLD.save(failureFileName, "IPTopo_list", IPTopo_list, "OpticalTopo_list", OpticalTopo_list,"IPScenarios_list",IPScenarios_list,"OpticalScenarios_list",OpticalScenarios_list)
            #     println("nohave!",failureFileName)
            # else
            #     println("have!",failureFileName)
            #     data = load(failureFileName)
            #     IPTopo_list, OpticalTopo_list, IPScenarios_list, OpticalScenarios_list = data["IPTopo_list"], data["OpticalTopo_list"], data["IPScenarios_list"], data["OpticalScenarios_list"]
            # end
            ...
            for algorithm in AllAlgorithms
                ...
                # if algorithm == "PATE_Benders_New_Tunnel" || algorithm == "PATE_New_Tunnel" || algorithm == "PATE_Benders" || algorithm == "PATE" || algorithm == "TEAVAR_PATE" || algorithm == "PATE_Benders_New_Tunnel_weak"
                # #if false    
                #     for state_id in 1:state_num
                #         start = time()
                #         IPTopo = IPTopo_list[state_id]
                #         OpticalTopo = OpticalTopo_list[state_id]
                #         IPScenarios = IPScenarios_list[state_id]
                #         OpticalScenarios = OpticalScenarios_list[state_id]   
                #         #rwa_scenario_restored_bw, rr_scenario_restored_bw, absolute_gap = abstract_optical_layer(GRB_ENV, IPTopo, OpticalTopo, IPScenarios, OpticalScenarios, scenario_generation_only, dir, topology, topology_index, verbose, scenario_cutoff, scenario_id, optical_rerouting_K, partial_load_full_tickets, option_num, large_option_num, option_gap, ticket_or_not, expanded_spectrum)
                ...
                #             ## tunnel routing
                #             if algorithm == "PATE_Benders_New_Tunnel" || algorithm == "PATE_New_Tunnel" || algorithm == "PATE_Benders_New_Tunnel_weak"
                #                 println("algorithm == New_Tunnel")
                #                 tunnelK_s = tunnelK+4
                #                 new_tunnel_or_not = 1
                #                 # T1, Tf1 = getTunnels(IPTopo, flows, tunnelK+3, 1, verbose, IPScenarios["code"], edge_disjoint=tunnel_rounting)
                #             else
                #                 tunnelK_s = tunnelK
                #                 new_tunnel_or_not = 0
                #                 # T1, Tf1 = getTunnels(IPTopo, flows, tunnelK, 0, verbose, IPScenarios["code"], edge_disjoint=tunnel_rounting)
                #             end
                            
                #             GT_time = time()
                #             T1, Tf1 = getTunnels(IPTopo, OpticalTopo, flows, tunnelK_s, new_tunnel_or_not, verbose, IPScenarios["code"], edge_disjoint=tunnel_rounting)
                #             GT_time = time() - GT_time
                #             println("Get_tunnel_time", GT_time)
                ...
                #                 TE_time = time()
                #                 TunnelBw, FlowBw, var, initial_throughput, TEruntime, best_options, best_scenario_resored_bw = TrafficEngineering(GRB_ENV, IPTopo, OpticalTopo, algorithm, IPTopo["links"], IPTopo["capacity"], demand, flows, T1, Tf1, IPScenarios["code"], OpticalScenarios["code"], IPScenarios["prob"], scenario_restored_bw, optical_rerouting_K, tunnelK_s, beta, absolute_gap, verbose, solve_or_not)
                #                 drawTunnel(topology, TunnelBw, T1, Tf1, IPTopo["links"], length(IPTopo["nodes"]), dir, algorithm)   # draw tunnel bandwidth graph
                #                 println("TE_time", time()-TE_time)

                #                 println("-----TrafficEngineering------")
                #                 # println("TunnelBw ", TunnelBw)
                #                 # println("FlowBw", FlowBw)
                #                 println("initial_throughput ", initial_throughput)
                #                 println("TEruntime ", TEruntime)
                #                 # println("var", var)
                #                 if verbose
                ...
                #                     links_utilization = computeLinksUtilization(IPTopo["links"], IPTopo["capacity"], demand, flows, T1, Tf1, tunnelK_s, TunnelBw)
                #                     losses, affected_flows, required_RouterPorts = TrafficReAssignment(links_utilization, IPTopo["links"], IPTopo["capacity"], demand, flows, T1, Tf1, tunnelK_s, TunnelBw, FlowBw, IPScenarios["code"], selected_scenario_restored_bw, algorithm, verbose)
                ...
                #                     println("compute_evaluation_time",time()-evaluation_time)
                                    
                #                     println("------------result-----------")
                #                     println("state_id and state_prob", state_id, state_prob[state_id])
                #                     println("scenario_availability ", scenario_availability)
                #                     println("flow_availability ", flow_availability)
                #                     println("bw_availability ", bw_availability)
                #                     println("conditional_scenario_availability ", conditional_scenario_availability)
                #                     println("conditional_flow_availability ", conditional_flow_availability)
                #                     println("conditional_bw_availability ", conditional_bw_availability)

                #                     if isdir("$(parallel_dir)/$(topology)/state_summary") == false
                #                         mkdir("$(parallel_dir)/$(topology)/state_summary")
                #                         if isdir("$(parallel_dir)/$(topology)/state_summary/$(algorithm)") == false
                #                             mkdir("$(parallel_dir)/$(topology)/state_summary/$(algorithm)")
                #                         end
                #                     else
                #                         if isdir("$(parallel_dir)/$(topology)/state_summary/$(algorithm)") == false
                #                             mkdir("$(parallel_dir)/$(topology)/state_summary/$(algorithm)")
                #                         end
                #                     end
                #                     temp_state_prob = state_prob[state_id]
                #                     open("$(parallel_dir)/$(topology)/state_summary/$(algorithm)/SUMMARY_$(topology_index)_$(scenario_id)_$(traffic_id)_$(scale_id)_$(beta)_$(state_id)_$(temp_state_prob).txt", "w+") do f_state
                #                         write(f_state, "Scenario_Availability\n")
                #                         writedlm(f_state, scenario_availability)
                
                #                         write(f_state, "Flow_Availability\n")
                #                         writedlm(f_state, flow_availability)
                
                #                         write(f_state, "Bandwidth_Availability\n")
                #                         writedlm(f_state, bw_availability)
                
                #                         write(f_state, "conditional_Scenario_Availability\n")
                #                         writedlm(f_state, conditional_scenario_availability)
                
                #                         write(f_state, "conditional_Flow_Availability\n")
                #                         writedlm(f_state, conditional_flow_availability)
                
                #                         write(f_state, "conditional_Bandwidth_Availability\n")
                #                         writedlm(f_state, conditional_bw_availability)
                #                     end

                #                     ## store results
                #                     Scenario_Availability[algorithm][traffic_num, s] += Float64(scenario_availability) * state_prob[state_id]
                #                     Flow_Availability[algorithm][traffic_num, s] += Float64(flow_availability) * state_prob[state_id]
                #                     Bandwidth_Availability[algorithm][traffic_num, s] += Float64(bw_availability) * state_prob[state_id]
                #                     conditional_Scenario_Availability[algorithm][traffic_num, s] += Float64(conditional_scenario_availability) * state_prob[state_id]
                #                     conditional_Flow_Availability[algorithm][traffic_num, s] += Float64(conditional_flow_availability) * state_prob[state_id]
                #                     conditional_Bandwidth_Availability[algorithm][traffic_num, s] += Float64(conditional_bw_availability) * state_prob[state_id]
                #                     Algo_LinksUtilization[algorithm][traffic_num, s, :] += Float64.(links_utilization) * state_prob[state_id]
                #                     Algo_RouterPorts[algorithm][traffic_num, s, :] += Float64.(required_RouterPorts) * state_prob[state_id]
                #                     Algo_accommodate_ratio[algorithm][traffic_num, s] += Float64(initial_throughput/sum(demand)) * state_prob[state_id]
                ...
                #             end
                #         end
                #         println("one_state_time",time()-start)
                #     end
                #     for traffic_num in 1:length(AllTraffic)
                #         for s in 1:length(scales)
                #             Scenario_Availability[algorithm][traffic_num, s] /= sum(state_prob)
                #             Flow_Availability[algorithm][traffic_num, s] /= sum(state_prob)
                #             Bandwidth_Availability[algorithm][traffic_num, s] /= sum(state_prob)
                #             conditional_Scenario_Availability[algorithm][traffic_num, s] /= sum(state_prob)
                #             conditional_Flow_Availability[algorithm][traffic_num, s] /= sum(state_prob)
                #             conditional_Bandwidth_Availability[algorithm][traffic_num, s] /= sum(state_prob)
                #             Algo_LinksUtilization[algorithm][traffic_num, s, :] /= sum(state_prob)
                #             Algo_RouterPorts[algorithm][traffic_num, s, :] /= sum(state_prob)
                #             Algo_accommodate_ratio[algorithm][traffic_num, s] /= sum(state_prob)
                #         end
                #     end
                # elseif algorithm == "TEAVAR_diff"
                #     for traffic_num in 1:length(AllTraffic)
                #         IPTopo, OpticalTopo, IPScenarios, OpticalScenarios = IPTopo_0, OpticalTopo_0, IPScenarios_0, OpticalScenarios_0
                #         # parsing demand from demand matrices
                #         initial_demand, initial_flows = readDemand("$(topology)/demand", length(IPTopo["nodes"]), AllTraffic[traffic_num], demand_upscale, demand_downscale, false)  # no rescaled
                #         rescaled_demand = initial_demand
                #         flows = initial_flows
                ...
                #         end
                #     end
                # elseif algorithm == "FloMore" || algorithm== "FloMore_Benders"
                #     for state_id in 1:state_num
                #         start = time()
                #         IPTopo = IPTopo_list[state_id]
                #         OpticalTopo = OpticalTopo_list[state_id]
                ...
                #                     evaluation_time = time()
                #                     ## post failure evaluation under different failure scenarios
                #                     links_utilization = computeLinksUtilization_flomore(IPTopo["links"], IPTopo["capacity"], demand, flows, T1, Tf1, tunnelK_s, TunnelBw)
                #                     losses, affected_flows, required_RouterPorts = TrafficReAssignment(links_utilization, IPTopo["links"], IPTopo["capacity"], demand, flows, T1, Tf1, tunnelK_s, TunnelBw, FlowBw, IPScenarios["code"], selected_scenario_restored_bw, algorithm, verbose)
                ...
                # else
                    for traffic_num in 1:length(AllTraffic)
                        ...
                        ## tunnel routing
                        if algorithm == "PATE_Benders_New_Tunnel" || algorithm == "PATE_New_Tunnel"
                            println("algorithm == New_Tunnel")
                            tunnelK_s = tunnelK+2
                            new_tunnel_or_not = 1
                            # T1, Tf1 = getTunnels(IPTopo, flows, tunnelK+3, 1, verbose, IPScenarios["code"], edge_disjoint=tunnel_rounting)
                        ...
                        #计算所有的tunnel的长度
                        # T1_length = []
                        # for ip_links_in_tunnel in T1
                        #     tunnel_length = 0
                        #     for link in ip_links_in_tunnel
                        #         tunnel_length += IPTopo["link_length"][link]
                        #     end
                        #     push!(T1_length, tunnel_length)
                        # end
                        println("T1: ", T1)
                        println("Tf1: ", Tf1)
                        println("flows: ", flows)
                        # println("T1_length: ", T1_length)
                        ...
                        for s in 1:length(scales)
                            ...
                            if failure_simulation  # if we run failure simulation after TE optimization
                                ...
                                #     OpticalScenarios = OpticalScenarios_list[state_id]   
                                    ...
                                    # println("TunnelBw_before ", TunnelBw)
                                    # if algorithm == "FloMore" || algorithm== "FloMore_Benders"
                                    ...
                                    # println("TunnelBw_after ", TunnelBw)
                                    println("hkl_test")
                                    links_utilization = computeLinksUtilization(IPTopo["links"], IPTopo["capacity"], demand, flows, T1, Tf1, tunnelK_s, TunnelBw)
                                    losses, affected_flows, required_RouterPorts = TrafficReAssignment(links_utilization, IPTopo["links"], IPTopo["capacity"], demand, flows, T1, Tf1, tunnelK_s, TunnelBw, FlowBw, IPScenarios["code"], selected_scenario_restored_bw, algorithm, verbose)
                                    ...
                                    println("------------result-----------",algorithm)#,state_id)
                                    # println("state_id and state_prob", state_id, state_prob[state_id])
                                    println("scenario_availability ", scenario_availability)
                                    println("flow_availability ", flow_availability)
                                    println("bw_availability ", bw_availability)
                ...
                DirectThroughput_Filename = "$(parallel_dir)/$(topology)/jld/$(algorithm)/Direct_Throughput_$(topology_index)_$(scenario_id)_$(traffic_id)_$(scale_id)_$(option_num)_$(beta)"
                JLD.save(DirectThroughput_Filename * ".jld", "DirectThroughput ", DirectThroughput)
                writedlm(DirectThroughput_Filename * ".txt", DirectThroughput)
                
                Algo_Runtime_Filename = "$(parallel_dir)/$(topology)/jld/$(algorithm)/Algo_Runtime_$(topology_index)_$(scenario_id)_$(traffic_id)_$(scale_id)_$(option_num)_$(beta)"
                JLD.save(Algo_Runtime_Filename * ".jld", "Algo_Runtime ", Algo_Runtime)
                writedlm(Algo_Runtime_Filename * ".txt", Algo_Runtime)

                if failure_simulation  # if we run failure simulation after TE optimization
                    ## write results to jld files for parallel processing
                    Scenario_Availability_FileName = "$(parallel_dir)/$(topology)/jld/$(algorithm)/Scenario_Availability_$(topology_index)_$(scenario_id)_$(traffic_id)_$(scale_id)_$(option_num)_$(beta)"
                    JLD.save(Scenario_Availability_FileName * ".jld", "Scenario_Availability ", Scenario_Availability)
                    writedlm(Scenario_Availability_FileName * ".txt", Scenario_Availability)
                    
                    Flow_Availability_FileName = "$(parallel_dir)/$(topology)/jld/$(algorithm)/Flow_Availability_$(topology_index)_$(scenario_id)_$(traffic_id)_$(scale_id)_$(option_num)_$(beta)"
                    JLD.save(Flow_Availability_FileName  * ".jld", "Flow_Availability ", Flow_Availability)
                    writedlm(Flow_Availability_FileName  * ".txt", Flow_Availability)

                    Bandwidth_Availability_FileName = "$(parallel_dir)/$(topology)/jld/$(algorithm)/Bandwidth_Availability_$(topology_index)_$(scenario_id)_$(traffic_id)_$(scale_id)_$(option_num)_$(beta)"
                    JLD.save(Bandwidth_Availability_FileName * ".jld", "Bandwidth_Availability ", Bandwidth_Availability)
                    writedlm(Bandwidth_Availability_FileName * ".txt", Bandwidth_Availability)

                    conditional_Scenario_Availability_FileName = "$(parallel_dir)/$(topology)/jld/$(algorithm)/conditional_Scenario_Availability_$(topology_index)_$(scenario_id)_$(traffic_id)_$(scale_id)_$(option_num)_$(beta)"
                    JLD.save(conditional_Scenario_Availability_FileName * ".jld", "conditional_Scenario_Availability ", conditional_Scenario_Availability)
                    writedlm(conditional_Scenario_Availability_FileName * ".txt", conditional_Scenario_Availability)

                    conditional_Flow_Availability_FileName = "$(parallel_dir)/$(topology)/jld/$(algorithm)/conditional_Flow_Availability_$(topology_index)_$(scenario_id)_$(traffic_id)_$(scale_id)_$(option_num)_$(beta)"
                    JLD.save(conditional_Flow_Availability_FileName * ".jld", "conditional_Flow_Availability ", conditional_Flow_Availability)
                    writedlm(conditional_Flow_Availability_FileName * ".txt", conditional_Flow_Availability)

                    conditional_Bandwidth_Availability_FileName = "$(parallel_dir)/$(topology)/jld/$(algorithm)/conditional_Bandwidth_Availability_$(topology_index)_$(scenario_id)_$(traffic_id)_$(scale_id)_$(option_num)_$(beta)"
                    JLD.save(conditional_Bandwidth_Availability_FileName * ".jld", "conditional_Bandwidth_Availability ", conditional_Bandwidth_Availability)
                    writedlm(conditional_Flow_Availability_FileName * ".txt", conditional_Flow_Availability)

                    SecureThroughput_Filename = "$(parallel_dir)/$(topology)/jld/$(algorithm)/Secure_Throughput_$(topology_index)_$(scenario_id)_$(traffic_id)_$(scale_id)_$(option_num)_$(beta)"
                    JLD.save(SecureThroughput_Filename * ".jld", "SecureThroughput ", SecureThroughput)
                    writedlm(SecureThroughput_Filename * ".txt", SecureThroughput)

                    Algo_RouterPorts_Filename = "$(parallel_dir)/$(topology)/jld/$(algorithm)/Algo_RouterPorts_$(topology_index)_$(scenario_id)_$(traffic_id)_$(scale_id)_$(option_num)_$(beta)"
                    JLD.save(Algo_RouterPorts_Filename * ".jld", "Algo_RouterPorts ", 0.0)
                    writedlm(Algo_RouterPorts_Filename * ".txt", 0.0)
                    
                    if isdir("$(parallel_dir)/$(topology)/summary") == false
                        mkdir("$(parallel_dir)/$(topology)/summary")
                        if isdir("$(parallel_dir)/$(topology)/summary/$(algorithm)") == false
                            mkdir("$(parallel_dir)/$(topology)/summary/$(algorithm)")
                        end
                    else
                        if isdir("$(parallel_dir)/$(topology)/summary/$(algorithm)") == false
                            mkdir("$(parallel_dir)/$(topology)/summary/$(algorithm)")
                    ...
                    open("$(parallel_dir)/$(topology)/summary/$(algorithm)/SUMMARY_$(topology_index)_$(scenario_id)_$(traffic_id)_$(scale_id)_$(beta).txt", "w+") do f
                        write(f, "DirectThroughput\n")
                        writedlm(f, DirectThroughput)

                        write(f, "Algo_Runtime\n")
                        writedlm(f, Algo_Runtime)
                        # write(f, "topology_index = $(topology_index), \nscenario_id = $(scenario_id), \ntraffic_id = $(traffic_id), \nscale_id = $(scale_id), \noptical_failure_prob_idx = $(optical_failure_prob_idx), \noptical_failure_prob = $(optical_failure_prob), \nbeta = $(beta)")
                        
                        write(f, "Scenario_Availability\n")
                        writedlm(f, Scenario_Availability)

                        write(f, "Flow_Availability\n")
                        writedlm(f, Flow_Availability)

                        write(f, "Bandwidth_Availability\n")
                        writedlm(f, Bandwidth_Availability)

                        write(f, "conditional_Scenario_Availability\n")
                        writedlm(f, conditional_Scenario_Availability)

                        write(f, "conditional_Flow_Availability\n")
                        writedlm(f, conditional_Flow_Availability)

                        write(f, "conditional_Bandwidth_Availability\n")
                        writedlm(f, conditional_Bandwidth_Availability)

                        write(f, "conditional_Flow_Availability\n")
                        writedlm(f, conditional_Flow_Availability)

                        write(f, "SecureThroughput\n")
                        writedlm(f, SecureThroughput)
                        
                        # write(f, "Algo_RouterPorts")
                        # writedlm(f, Algo_RouterPorts)
                    end
