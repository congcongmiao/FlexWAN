using Compose, GraphPlot, Gadfly, Graphs
# using Cairo
...
function drawTunnel(topology, a, T, Tf, edges, num_nodes, dir, algorithm)
    ...
    #     Graphs.add_edge!(graph, edges[e][1], edges[e][2])
    #     s = 0
    #     for f in 1:size(a,1)
    #         for t in 1:size(a,2)
    #             s += a[f,t] * L[Tf[f][t],e]
...
function drawGraph(topology, IPTopo, topology_index, OpticalTopo, dir)
    # #draw begin
    # graph = Graphs.SimpleDiGraph(length(IPTopo["nodes"]))
    # for e in 1:length(IPTopo["links"])
    #     Graphs.add_edge!(graph, IPTopo["links"][e][1], IPTopo["links"][e][2])
    ...
    #         if Graphs.has_edge(graph, ss, dd)
    #             for e in 1:length(IPTopo["links"])
    ...
    #     end
    # end
    # println("iplink_bw:", iplink_bw)
    # nodelabel = collect(1:length(IPTopo["nodes"]))
    # Compose.draw(PNG("$(dir)/$(topology)/00_IP_topo_graph.png", 1000, 1000), gplot(graph, NODELABELSIZE=20.0, nodelabel=nodelabel, arrowlengthfrac=.04))
    # #draw end
    ...
    # optical_graph = Graphs.SimpleDiGraph(length(OpticalTopo["nodes"]))
    # for e in 1:length(OpticalTopo["links"])
    #     Graphs.add_edge!(optical_graph, OpticalTopo["links"][e][1], OpticalTopo["links"][e][2])
    # end
    ...
    #         if Graphs.has_edge(optical_graph, ss, dd)
    #             for e in 1:length(OpticalTopo["links"])
