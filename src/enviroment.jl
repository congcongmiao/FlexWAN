...
function GetAllScenarios(IPTopo, OpticalTopo, cutoff, is_ffc_scenario, ffc_fiber_cut_num)
    if is_ffc_scenario
        ...
        # failure states #1111 --> #0111, 1011,1101,...
        for bits in collect(combinations(1:n_unifibers,ffc_fiber_cut_num)) #C(k,n)
            s = ones(n_unifibers)
            current_prob = 1
            for bit in bits
                s[bit] = 0
    ...
    println("scenarios, probs", scenarios, probs)
    ...
    println("oscenarios, oprobs", oscenarios, oprobs)
...
function subScenarios(optical_topo, IPTopo, cutoff; first=true, last=true, progress=true)
    ...
    println("without normalized probabilities: ", probabilities)
...
function subScenariosRecursion(optical_topo, IPTopo, original, cutoff, remaining=[], offset=0, partial=[], scenarios=[], probabilities=[]; progress=nothing)
    if (size(partial,1) == 0)   #first
        ...
        # println(" product >= cutoff:", product ,",", cutoff)
        if product >= cutoff
            # println("product >= cutoff true")
            if OpticalGraphConnectivity(optical_topo, bitmap)  # remove fiber cut events that disconnect fiber topo
                ...
                # println("iscenarios:",iscenarios)
                # if IPGraphConnectivity(IPTopo,iscenarios)
                #     println("IPGraphConnectivity(IPTopo,iscenarios)", ",true")
                # else
                #     println("IPGraphConnectivity(IPTopo,iscenarios) false")
                # end
                if IPGraphConnectivity(IPTopo,iscenarios)
                    # println("test1")
