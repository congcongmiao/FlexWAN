using JuMP, Combinatorics, Gurobi, TickTock

include("./algorithms/TEAVAR.jl")
include("./algorithms/FloMore.jl")
include("./algorithms/FloMore_Benders.jl")
include("./algorithms/PATE.jl")
include("./algorithms/PATE_Benders.jl")
...
# include("./optical_net_planning.jl")
...
function TrafficEngineering(GRB_ENV, IPTopo, OpticalTopo, algorithm, links, capacity, demand, flows, T, Tf, scenarios, fiber_scenarios, scenario_probs, scenario_restored_bw, optical_rerouting_K, tunnelK, beta, purb, verbose, solve_or_not)
    ...
    if algorithm == "TEAVAR" || algorithm == "TEAVAR_diff" || algorithm == "TEAVAR_PATE"
    ...
    elseif algorithm == "FloMore"
        alpha, TunnelBw, loss, solve_t, opt_t = FloMore(GRB_ENV, links, capacity, flows, demand, beta, T, Tf, scenarios, scenario_probs, average=true)
        for f in 1:nflows
            FlowBw[f] = demand[f] * (1-loss[f])
        end
        throughput = sum(FlowBw)
        solve_runtime = solve_t

    elseif algorithm == "FloMore_Benders"
        alpha, TunnelBw, loss, solve_t, opt_t = FloMore_Benders(GRB_ENV, links, capacity, flows, demand, beta, T, Tf, scenarios, scenario_probs, average=true)
        for f in 1:nflows
            FlowBw[f] = demand[f] * (1-loss[f])
        end
        throughput = sum(FlowBw)
    ...
    elseif algorithm == "PATE" || algorithm == "PATE_New_Tunnel"
        alpha, TunnelBw, loss, solve_t, opt_t = PATE(GRB_ENV, links, capacity, flows, demand, beta, T, Tf, scenarios, scenario_probs, average=true)
        for f in 1:nflows
            FlowBw[f] = demand[f] * (1-loss[f])
        end
        throughput = sum(FlowBw)
    ...
    elseif algorithm == "SMORE"
        alpha, TunnelBw, solve_t, opt_t = SMORE(GRB_ENV, links, capacity, flows, demand, T, Tf)
        throughput = sum(TunnelBw)
        solve_runtime = solve_t

    elseif algorithm == "PATE_Benders" || algorithm == "PATE_Benders_New_Tunnel" || algorithm == "PATE_Benders_diff" || algorithm == "PATE_Benders_Compare"
        alpha, TunnelBw, loss, solve_t, opt_t = PATE_Benders(GRB_ENV, links, capacity, flows, demand, beta, T, Tf, scenarios, scenario_probs, average=true)
        for f in 1:nflows
            FlowBw[f] = demand[f] * (1-loss[f])
        end
        throughput = sum(FlowBw)
    ...
    elseif algorithm == "ARROW"   # ARROW TE
        ...
        input_flexgrid_optical = readdlm("./data/flexgrid_optical.txt", header=true)[1]
        ...
        for s in 1:nscenarios
            ...
            # println("best_restore_bw[IP link-capacity],best_restore_bw_length: ", length(best_restore_bw),best_restore_bw)
            # modulation=[]
            # wavelength=[]
            # for i in 1:length(best_restore_bw)
            #     current_modulation, current_wavelength = flexgrid_plan(best_restore_bw[i], IPTopo["link_length"][i], input_flexgrid_optical) #3000,1500   1000,500  demand[i], links_length[i]
            #     println("current IP link modulation:",current_modulation)
            #     println("current IP link wavelength, sum(wavelength):", current_wavelength, ",", sum(current_wavelength), "\n")
            #     # println(" ")
            #     push!(modulation, current_modulation)
            #     push!(wavelength, current_wavelength)
            # end

            # topology = "B4"
            # topodir = "./data/topology/$(topology)"
            # input_topology_o = readdlm("$(topodir)/optical_topo.txt", header=true)[1]
            # input_topology_ip = readdlm("$(topodir)/IP_topo_1/IP_topo_1.txt", header=true)[1]
            # spectrum_center_storage, spectrum_width_storage = wavelength_assign(input_topology_o, input_topology_ip, false, "./data/topology", topology, wavelength, modulation, [], input_topology_ip[:,4], true, false)
