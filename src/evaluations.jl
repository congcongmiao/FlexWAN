...
function TrafficReAssignment(links_utilization, edges, capacity, demand, flows, T, Tf, k, TunnelBw, FlowBw, scenarios, restored, algorithm, verbose; progress=false)
    if algorithm == "FloMore" || algorithm == "FloMore_Benders"
        ...
        RouterPorts_all_s = deepcopy(links_utilization)
        arrow_algorithms = ["ARROW", "ARROW_NAIVE", "ARROW_BIN"]
        println("TunnelBw size", size(TunnelBw))
        ...
        for s in 1:nscenarios
            for t in 1:ntunnels
                if size(T[t],1) == 0
                    ...
                    # if length(scenarios[s]) != nedges
                    #     println("s: ", s)
                    #     println("scenarios[s][e]: ", length(scenarios[s]))
                    # end
                    # if length(restored[s]) != nedges
                    #     println("s: ", s)
                    #     println("restored[s][e]: ", length(restored[s]))
                    # end
                    # println("s: ", s, "e: ", e)
                    for e in 1:nedges
        ...
        println("RouterPorts_all_s ", RouterPorts_all_s)
        println("RouterPorts_all_s size", size(RouterPorts_all_s))
        println("nscenarios ", nscenarios)
        for s in 1:nscenarios
            # println("s ", s)
            RouterPorts = RouterPorts_all_s[s]
            if verbose printstyled("== Process scenario $(s)\n", color=:yellow) end
            scenario_network_capacity = 0
            scenario_tunnel_bw = deepcopy(TunnelBw[:,:,s])  ## 132x4xsth every failure scenario should start with the initial TE splitratio results
            ...
            for f in 1:nflows
                # if occursin("PATE_Benders_New_Tunnel", algorithm) || occursin("PATE_Benders", algorithm)  || occursin("PATE", algorithm)  || occursin("TEAVAR", algorithm)  || occursin("FFC", algorithm) || occursin("ARROW", algorithm) || occursin("ARROW_NAIVE", algorithm) || occursin("PATE_Benders_New_Tunnel_weak", algorithm)  # reservation based TE --> weak
                if occursin("PATE_Benders_New_Tunnel", algorithm) || occursin("FloMore", algorithm) || occursin("FloMore_Benders", algorithm) || occursin("PATE_Benders_Compare", algorithm) || occursin("PATE_Benders", algorithm)  || occursin("PATE", algorithm)  || occursin("TEAVAR", algorithm)  || occursin("ARROW", algorithm) || occursin("ARROW_NAIVE", algorithm)  # reservation based TE --> weak
                #if occursin("FFC", algorithm) || occursin("ARROW", algorithm) || occursin("ARROW_NAIVE", algorithm)
                # if occursin("ARROW", algorithm) || occursin("ARROW_NAIVE", algorithm)
                    ...
                    if scenario_flow_bw[f] < demand[f]  # assign overflow traffic to 0 unallocated tunnels
                        ...
                        if sum(zero_unallocated_tunnel) > 0  # there exist 0 unallocated tunnel
                            ...
                            for t in 1:size(Tf[f],1)
                                if zero_unallocated_tunnel[t] == 1
                                    scenario_tunnel_bw[f,t] = round(overflow/sum(zero_unallocated_tunnel), digits=16)  # actual allocation without reservation
                                    # println("zero_unallocated_tunnel length: ",T1_length[t])
                                end
                            end
                        end
                    end
...
function computeLinksUtilization_flomore(edges, capacity, demand, flows, T, Tf, k, splittingratios_original; progress=false)
    nedges = length(edges)
    nflows = length(flows)
    ntunnels = length(T)
    links_utilization = []
    ...
    for s in 1:size(splittingratios_original)[3]

        splittingratios = deepcopy(splittingratios_original[:,:,s])
        ...
        links_utilization_per_s = [0.0 for e in 1:nedges]
        for e in 1:nedges
            e_utilization = sum(routed[f,t] * L[Tf[f][t],e] for f in 1:nflows for t in 1:size(Tf[f],1))
            links_utilization_per_s[e] = e_utilization
        end
        push!(links_utilization, links_utilization_per_s)
    end
...
function computeLinksUtilization(edges, capacity, demand, flows, T, Tf, k, splittingratios_original; progress=false)
    ...
    println("links_utilization ", links_utilization)
    println("links_utilization size", size(links_utilization))
    println("links_utilization length", length(links_utilization))
    return links_utilization
end
